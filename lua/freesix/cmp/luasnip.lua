...
local cfg = pRequire("freesix").config.cmp

local pathUtils = require("freesix.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
    paths = pathUtils.join(pathUtils.getConfig(), "lua", "freesix", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
    paths = pathUtils.join(pathUtils.getConfig(), "lua", "freesix", "cmp", "snippets", "vscode"),
})
